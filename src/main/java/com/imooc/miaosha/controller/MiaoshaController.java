...
public class MiaoshaController implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        for(GoodsVo goods : goodsList) {
            ...
            localOverMap.put(goods.getId(), false);//本地记录，没有卖完。
        }
    }
	...
	public Result<Integer> miaosha(Model model,MiaoshaUser user, @RequestParam("goodsId")long goodsId) {
		...
		if(stock < 0) {
			localOverMap.put(goodsId, true);//本地记录，卖完了
			return Result.error(CodeMsg.MIAO_SHA_OVER);
		}
		//判断是否已经秒杀到了
     ...
     * @return
     */
    @AccessLimit(seconds=5, maxCount=5, needLogin=true)//自定义拦截器，接口限流，5秒钟最多访问5次
